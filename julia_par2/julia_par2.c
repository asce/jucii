...
int main(int argc, char* argv[]) {
	...
	if(rank==0){//MASTER
	  ...
	  while(written_rows < rows_number){
	    //Recibe numero de fila (en TAG) y fila
	    MPI_Recv(row_buff,row_size,MPI_CHAR,MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD,&status);
	    row = status.MPI_TAG;
	  ...
	  while(finished == 0){
		...
		getchar();
	      ...
	      MPI_Send(row_buff,row_size,MPI_CHAR,0,row,MPI_COMM_WORLD);
	    }
	  }
	}
